...
import { ClaimPayerStatus, ClaimProviderStatus, ClaimServiceType } from '../../../../../billing/dashboard/v3/types';
...
const ClaimVisits: React.FC<claimVisitsProps> = ({ locationUuid, billingDate, onDateChange }) => {
  ...
  const serviceTypeOptions = Object.values(ClaimServiceType).map((s) => {
    return {
      text: s,
      id: s,
    };
  });
  const filteredClaimVisits = useMemo(
    () => filterClaimsVisits(claimVisits ?? []),
    [claimVisits, selectedProviderStatus, selectedPayerStatus, searchString, selectedServiceType],
  );
  useEffect(() => {
  ...
  function serviceTypeChangeHandler(selectedStatus: { selectedItem: { id: string; text: string } }) {
    let serviceType = '';
    if (selectedStatus && selectedStatus.selectedItem) {
      serviceType = selectedStatus.selectedItem.id;
    }
  ...
  function filterClaimsVisits(claimVisits: ClaimVisitReponse[]) {
    return claimVisits
      ...
      .filter((b) => {
        ...
        return b.serviceType === selectedServiceType;
      });
  }
      ...
      <div className={styles.claimVisitsLayout}>
        <div className={styles.filterRow}>
          ...
          <div className={styles.actionCol}>
            ...
            <Button kind="tertiary" onClick={handleRefresh}>
              Refresh
            </Button>
          </div>
        </div>
        <div>
          <Table aria-label="claim visits" size="sm">
            ...
            <TableBody>
              {filteredClaimVisits &&
                filteredClaimVisits.map((cv, index) => {
                  return (
                    <>
                      <TableRow key={cv.id}>
